<form class="pr-2" id="yes-form" action="{{ route('answers') }}" method="POST">
    ...
    @if( isset($nextId) && $nextId != null )
        <input name="next" type="hidden" value="{{ $nextId }}">
    @endif
...
<form class="pl-2" id="skip-form" action="{{ route('answers') }}" method="POST">
    ...
    @if( isset($nextId) && $nextId != null )
        <input name="next" type="hidden" value="{{ $nextId }}">
    @endif
    <button accesskey="s" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" title="Press 2 to execute">Skip [2]</button>
</form>
<form id="no-form" action="{{ route('answers') }}" method="POST">
    ...
    @if( isset($nextId) && $nextId != null )
        <input name="next" type="hidden" value="{{ $nextId }}">
    @endif
    <button accesskey="n" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" title="Press 3 to execute">No [3]</button>
</form>
...
class KeyboardActions {
	...
	actionMapping(character) {
		...
		switch (character) {
			...
			case '2': action = 'skip'; break;
			case '3': action = 'no'; break;
		}
		return action;
	}
}
