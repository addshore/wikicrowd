    ...
    template: `
      <div class='flex flex-col items-center w-full'>
        <div class='w-full max-w-xl p-6'>
          <h1 class='text-2xl font-bold mb-4 text-center'>Custom Depicts Grid</h1>
          <form @submit.prevent="showGrid = true" class='mb-6 flex flex-col gap-4'>
            <div>
              <label class='block font-semibold mb-1'>Commons Category</label>
              <input v-model="manualCategory" class='border rounded px-2 py-1 w-full' placeholder='e.g. Paintings by Vincent van Gogh' required />
            </div>
            <div>
              <label class='block font-semibold mb-1'>Wikidata Qid</label>
              <input v-model="manualQid" class='border rounded px-2 py-1 w-full' placeholder='e.g. Q5582' required />
            </div>
            <button type='submit' class='bg-blue-600 text-white px-4 py-2 rounded font-bold'>Generate Grid</button>
          </form>
        </div>
        <div class='w-full'>
